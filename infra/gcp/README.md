...
the production Next build, pushes the image to Artifact Registry, and deploys its immutable
...
gcloud compute ssh fleet-research --project=fleet-governance --zone=us-central1-a \
  --tunnel-through-iap -- -N -L 3100:127.0.0.1:3100
```

Then open `http://localhost:3100`. The browser is local; the server, agents and tools run on GCP.
