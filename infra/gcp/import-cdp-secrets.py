...
def credentials_from_json(raw):
    try:
        data = json.loads(raw)
    except (ValueError, TypeError):
        raise ImportFailure('Missing or invalid CDP bootstrap JSON.') from None
    if not isinstance(data, dict) or set(data) != set(FIELDS):
        raise ImportFailure('Expected exactly api_key_id and api_key_secret.')
...
def main():
    ...
    values = credentials_from_json(os.environ.pop('CDP_BOOTSTRAP_CREDENTIALS', ''))
    ...
    if summary:
        with Path(summary).open('a') as stream:
            stream.write('\nCDP credentials imported and read back successfully.\n\n')
...
if __name__ == '__main__':
    try:
        ...
        print('CDP import failed unexpectedly; diagnostic values withheld.', file=sys.stderr)
        sys.exit(1)
