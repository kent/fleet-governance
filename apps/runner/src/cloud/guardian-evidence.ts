...
export function guardianObservation(allocation: ComputeAllocation, observation: ComputeObservation | null, state: ComputeState,
  ...
  const proposals = observation?.proposals.filter(p => Number.isInteger(p.state) && p.state >= (allocation.checkpoints ? -1 : 0) && p.state <= 7) ?? [];
  return { at: now, phase: state.phase, ...(observation ? { blockNumber: observation.blockNumber } : {}), proposals,
    vmStatus: vm.status, checks: [
      ...
      { name: "Required approval", status: state.reason === "vote_failed" ? "fail" : state.phase === "authorised" && !state.waitingForProposal ? "pass" : state.phase === "voting" || state.waitingForProposal ? "pending" : "unknown", detail: state.reason === "vote_failed" ? `Required proposal ${state.failedProposalId} failed` : allocation.checkpoints ? `${state.approvedProposalIds?.length ?? 0}/${allocation.checkpoints.length} checkpoints executed. ${state.waitingForProposal ? "Only prior approved work and initial lab tools may run." : "Task dispatch waits while the next vote is pending."}` : state.phase === "authorised" ? "Required proposals executed" : "Task execution waits for settled approval" },
      check("Fixed expiry", now < allocation.stopAt && (allocation.checkpoints ? allocation.checkpoints.every(c => now < c.approvalDeadline || state.approvedProposalIds?.includes(c.proposalId)) : now < allocation.approvalDeadline || state.phase === "authorised"), `Approval deadline ${new Date(allocation.approvalDeadline * 1000).toISOString()} · hard stop ${new Date(allocation.stopAt * 1000).toISOString()}`),
    ] };
}
