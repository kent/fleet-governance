...
export function guardianObservation(allocation: ComputeAllocation, observation: ComputeObservation | null, state: ComputeState,
  ...
  return { at: now, phase: state.phase, ...(observation ? { blockNumber: observation.blockNumber } : {}), proposals,
    vmStatus: vm.status, checks: [
      ...
      allocation.discovery ? check(allocation.discovery.proposalToken ? "ERC-20 proposal burns" : "Agent proposals and credits", observation ? !!observation.discoveryVerified && proposals.every(p => p.creditPaid) : null,
        `Task ${allocation.discovery.taskId} · ${proposals.length} discovered proposal(s) · ${allocation.discovery.creditsPerAgent} ${allocation.discovery.proposalToken ? "FPROP per agent; token supply and balances verified onchain" : "credits per agent"}`)
      ...
      check("Fixed expiry", now < allocation.stopAt && (allocation.discovery ? true : allocation.checkpoints ? allocation.checkpoints.every(c => now < c.approvalDeadline || state.approvedProposalIds?.includes(c.proposalId)) : now < allocation.approvalDeadline || state.phase === "authorised"), allocation.discovery
        ? `Each agent proposal has ${allocation.discovery.proposalWindowSeconds}s from its ${allocation.discovery.proposalToken ? "atomic proposal transaction" : "credit payment"}. Hard stop ${new Date(allocation.stopAt * 1000).toISOString()}`
