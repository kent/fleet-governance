...
export function guardianObservation(allocation: ComputeAllocation, observation: ComputeObservation | null, state: ComputeState,
  ...
  return { at: now, phase: state.phase, ...(observation ? { blockNumber: observation.blockNumber } : {}), proposals,
    vmStatus: vm.status, checks: [
      ...
      allocation.discovery ? check(allocation.discovery.proposalBonds ? "FleetGov proposal bonds" : allocation.discovery.proposalToken ? "ERC-20 proposal burns" : "Agent proposals and credits", observation ? !!observation.discoveryVerified && proposals.every(p => p.creditPaid) : null,
        `Task ${allocation.discovery.taskId} · ${proposals.length} discovered proposal(s) · ${allocation.discovery.proposalBonds ? "FleetGov reservations, forfeitures and fixed supply verified onchain" : `${allocation.discovery.creditsPerAgent} ${allocation.discovery.proposalToken ? "FPROP per agent; token supply and balances verified onchain" : "credits per agent"}`}`)
      ...
      { name: "Required approval", status: state.reason === "vote_failed" || state.reason === "fleet_voted_stop" ? "fail" : state.phase === "authorised" ? "pass" : state.phase === "voting" ? "pending" : "unknown", detail: state.reason === "fleet_voted_stop" ? `Stop motion ${state.failedProposalId} passed. The fleet voted to stop` : state.reason === "vote_failed" ? `Required proposal ${state.failedProposalId} failed` : allocation.discovery ? `${state.approvedProposalIds?.length ?? 0}/${proposals.length} agent proposals executed. ${state.phase === "authorised" ? "Initial scope and exact approved requests may run." : "Task dispatch waits for every pending proposal."}` : allocation.checkpoints ? `${state.approvedProposalIds?.length ?? 0}/${allocation.checkpoints.length} checkpoints executed. ${state.waitingForProposal ? "Only prior approved work and initial lab tools may run." : state.phase === "authorised" ? "All planned checkpoints executed within the fixed allocation." : "Task dispatch waits while the next vote is pending."}` : state.phase === "authorised" ? "Required proposals executed" : "Task execution waits for settled approval" },
      check("Fixed expiry", now < allocation.stopAt && (allocation.discovery ? true : allocation.checkpoints ? allocation.checkpoints.every(c => now < c.approvalDeadline || state.approvedProposalIds?.includes(c.proposalId)) : now < allocation.approvalDeadline || state.phase === "authorised"), allocation.discovery
        ? `Each agent proposal has ${allocation.discovery.proposalWindowSeconds}s from its ${allocation.discovery.proposalToken || allocation.discovery.proposalBonds ? "atomic proposal transaction" : "credit payment"}. Hard stop ${new Date(allocation.stopAt * 1000).toISOString()}`
