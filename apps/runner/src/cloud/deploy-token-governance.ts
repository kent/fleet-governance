...
async function deploy() {
  ...
  await writeControlObject("contracts/token-governance-v3-claim.json", { revision: process.env.GITHUB_SHA, workflowRun: process.env.GITHUB_RUN_ID });
  const run = spawnSync("docker", ["run", "--rm", "--user", "0:0", "-v", `${process.cwd()}:/workspace`, "-w", "/workspace/contracts",
    ...
    "script", "script/DeployFleet.s.sol", "--broadcast", "--slow", "--non-interactive", "--gas-price", "10000000"], {
