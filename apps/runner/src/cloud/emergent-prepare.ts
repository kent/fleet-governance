...
export async function prepareEmergent(input: { request: SimulationRequest; client: FleetClient; rpcUrl: string;
  ...
  await progress({ component: "compute", type: "allocation.armed", title: "One FleetGov token. Agent-authored proposals.",
    detail: `Each agent holds one FleetGov voting token. Proposing reserves ${settings.proposalBond} FleetGov and requires ${settings.proposalThreshold} voting units. A ${settings.proposalCooldownSeconds}s cooldown and one unsettled proposal per agent limit spam. A proposal can ask for access or move to stop the whole fleet; a passed stop motion or a defeated request stops this VM. The bond returns when ${settings.bondParticipationPercent}% of the total supply votes FOR, AGAINST or ABSTAIN, even if the proposal loses. Cancellation or insufficient participation forfeits it. Fresh experiments redistribute the original fixed supply after the old run closes; no new tokens are minted.`,
