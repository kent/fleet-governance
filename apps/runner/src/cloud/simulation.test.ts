...
beforeEach(() => { vi.resetAllMocks(); mocks.control.mockResolvedValue(null); mocks.allocation.mockResolvedValue(null); mocks.read.mockResolvedValue(null); mocks.request.mockResolvedValue({ json: async () => ({ status: "RUNNING" }) }); });
describe("real simulation request", () => {
  it("creates protected reservation and invokes only the fixed job with no overrides", async () => {
    ...
    expect(mocks.request.mock.calls[0]?.[1]).toContain("ifGenerationMatch=0");
  ...
  it("retains the protected request after an ambiguous dispatch and never restarts on an authority race", async () => {
    ...
    expect(mocks.request.mock.calls[0]?.[2]?.method).toBe("POST");
  });
});
