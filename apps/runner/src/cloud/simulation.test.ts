...
describe("real simulation request", () => {
  it("allows read-only inspection without granting a removed operator launch authority", async () => {
    ...
    await expect(readRecordedSimulationRequest()).resolves.toEqual({ ...saved, settings: ExperimentSettings.parse(saved.settings) });
  ...
  it("refuses to start during human recovery while the shutdown controller is absent", async () => {
    ...
    await expect(queueSimulation(runId, experimentDefaults(), "operator2@example.com")).rejects.toThrow("not ready");
    expect(mocks.write).not.toHaveBeenCalled();
  ...
  it("rejects a previously retired run identity before creating a request or starting anything", async () => {
    ...
    await expect(queueSimulation(runId, experimentDefaults(), "operator2@example.com")).rejects.toThrow("permanently retired");
    expect(mocks.request).not.toHaveBeenCalled();
  });
  it("never repeats the job for a retried request or permits another request to replace it", async () => {
    ...
    await queueSimulation(runId, experimentDefaults(), "operator2@example.com");
    expect(mocks.request).not.toHaveBeenCalled();
    await expect(queueSimulation("run-00000000-0000-4000-8000-000000000002", experimentDefaults(), "operator2@example.com")).rejects.toThrow("recovery");
  });
  it("blocks a halted allocation and an active ordinary experiment before any mutation", async () => {
    ...
    await expect(queueSimulation(runId, experimentDefaults(), "operator2@example.com")).rejects.toThrow("recovery");
    expect(mocks.request).not.toHaveBeenCalled();
    ...
    await expect(queueSimulation(runId, experimentDefaults(), "operator2@example.com")).rejects.toThrow("already running");
    expect(mocks.request).not.toHaveBeenCalled();
  });
  it("retains the protected request after an ambiguous dispatch and never restarts on an authority race", async () => {
    ...
    await expect(queueSimulation(runId, experimentDefaults(), "operator2@example.com")).rejects.toThrow("concurrently");
