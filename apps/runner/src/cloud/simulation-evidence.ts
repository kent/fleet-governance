...
try {
  ...
  if (votes.length !== 5 || outcome !== ProposalState.Defeated || state?.value.reason !== "vote_failed" || state.value.failedProposalId !== work.proposalId || !state.value.stoppedAt || vm.id !== allocation.instanceId || vm.status !== "TERMINATED") throw new Error("Real shutdown acceptance checks did not pass.");
  ...
  const evidence = { ...progress, ...work, scripted: false, outcome: "Defeated", allocation, controller: state.value, vm: { id: vm.id, status: vm.status },
  ...
  console.log(JSON.stringify({ event: "real_shutdown_verified", runId: request.runId, proposalId: work.proposalId, votes: votes.length, vmStatus: vm.status, controllerReason: state.value.reason, restartDenied }));
} catch {
