...
describe("GET /api/runs/[id]/record", () => {
  it.each(["..", "../../etc/passwd", "a/b", "a%2fb", ""])("rejects the invalid run id %j with 400, echoing nothing", async (badId) => {
    const res = await GET(new Request("http://localhost/api/runs/x/record"), ctx(badId));
  ...
  it("passes id validation for a normal id and reaches the real not-found check", async () => {
    const res = await GET(new Request("http://localhost/api/runs/x/record"), ctx("no-such-run-xyz"));
    // Proves it got past the id gate: a different status/error than "invalid run id".
